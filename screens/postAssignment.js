...
const PostAssignment = () => {
  ...
  const goInsert = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.1.179:5001/api/createAssignment', insertData);
      ...
      const assignmentIDResponse = await fetch('http://192.168.1.179:5001/api/getAssignmentID');
      const assignmentIDData = await assignmentIDResponse.json()
      const maxAssID = assignmentIDData.maxAssignmentId

      const queryStudentResponse = await fetch(`http://192.168.1.179:5001/api/getStudent?classID=${classID}`);
      ...
      const generateStatusResponse = await axios.post('http://192.168.1.179:5001/api/generateStatus', { dataToInsert: studentEachClass });
