      ...
      renderItem={({item,index})=>(
        <View>
          ...
          flexDirection:'row',alignItems:'center',paddingTop:'5%'}}>
            <Text style={[customStyles.h2,{flex:1}]}>{truncate(item.assignment_name,maxLength)}</Text>
            <TouchableOpacity>
                <Image source={require('../../assets/icons/threedots.png')}></Image>
            </TouchableOpacity>
          </View>
          <View style={{display:'flex',
          flexDirection:'row',alignItems:'center',paddingBottom:'5%',paddingTop:'2%'}}>
            ...
            <View>
              <Text style={[customStyles.h1,customStyles.assignmentButton,
                {backgroundColor:status_dict[item.Submit_Count == item.Assigned_Count]}]}>
