...
const PlannerCalendar = ({day, setDay}) => {  // day = selected day
  ...
  const getDaysFromWeek = (week)=>{
      return week.map((item)=>{
          ...
          return (
            <Pressable key={current} style={[customStyles.calendarWidget,
            current === day? {backgroundColor:'rgba(207, 207, 252, 0.3)'}:{}]}
            onPress={()=>{
              setDay(current)
              }}>
              <Text style={customStyles.h4}>{displayDate[0]}</Text>
              <Text style={customStyles.bodySmall}>{displayDate[1].substring(0,2)}</Text>
              <View style={[{width:6,height:6,borderRadius:3,
                            margin:5},current === day? {backgroundColor:'#F04E22'}:
